...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute([$code]);
    if ($stmt->fetch()) {
        echo '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">كود البريد موجود مسبقاً، يرجى استخدام كود آخر</div>';
    } else {
        ...
        try {
            ...
            if ($stmt->execute([$date, $to_branch_id, $code, $description, $sender_id, $receiver_id, $handed_to])) {
                echo '<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert">تم إضافة البريد الصادر بنجاح</div>';
            }
        } catch (PDOException $e) {
            if ($e->errorInfo[1] == 1062) { // MySQL duplicate entry error code
                echo '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">كود البريد موجود مسبقاً، يرجى استخدام كود آخر</div>';
            } else {
                echo '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">حدث خطأ أثناء حفظ البيانات: ' . htmlspecialchars($e->getMessage()) . '</div>';
            }
        }
    }
}
...
$max_code = $pdo->query("SELECT MAX(code) FROM mail_out as max_code")->fetchColumn();
...
<h2 class="text-2xl font-bold mb-6">إضافة بريد صادر</h2>
...
<div class="mb-4" dir="rtl">
    <label for="code" class="block mb-2 font-medium text-right">كود البريد</label>
    <div class="flex items-center gap-2 flex-row-reverse">
        <input type="text" id="code" name="code" required 
               value="<?= htmlspecialchars($next_code) ?>"
               class="flex-1 p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-right">
...
<h2 class="text-2xl font-bold mb-6">سجل البريد الصادر</h2>
...
<div class="hidden print:block text-center mb-4">
    <h1 class="text-2xl font-bold">سجل البريد الصادر</h1>
