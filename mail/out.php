...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['edit_id'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE mail_out SET date = ?, to_branch_id = ?, code = ?, description = ?, sender_id = ?, receiver_id = ?, handed_to = ? WHERE id = ?");
            if ($stmt->execute([$date, $to_branch_id, $code, $description, $sender_id, $receiver_id, $handed_to, $id])) {
                $_SESSION['success'] = "تم تحديث البريد الصادر بنجاح";
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        ...
        if ($stmt->execute([$id])) {
            $_SESSION['success'] = "تم حذف البريد الصادر بنجاح";
            header('Location: out.php');
            exit;
        }
    } catch (PDOException $e) {
...
if (isset($_GET['export'])) {
    ...
    if (!empty($search)) {
        $conditions[] = "(b.name LIKE ? OR mo.code LIKE ? OR s.name LIKE ? OR r.name LIKE ? OR mo.handed_to LIKE ? OR mo.description LIKE ?)";
        $searchTerm = "%$search%";
...
<div class="container mx-auto px-4 py-6">
        ...
        <form method="GET" class="flex gap-4 items-end">
            <div class="flex-1">
                ...
                <input type="text" id="search" name="search" value="<?= htmlspecialchars($search) ?>" 
                       ...
                       placeholder="ابحث في البريد الصادر...">
            </div>
            <div>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded">
                    بحث
                </button>
                <a href="out.php" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">
                    إعادة تعيين
                </a>
            </div>
        </form>
    ...
    <i class="fas fa-edit"></i>
                        ...
                        <button onclick="if(confirm('هل أنت متأكد من حذف هذا البريد؟')) window.location.href='?delete_id=<?= $item['id'] ?>'" 
                           class="text-red-600 hover:text-red-800"><i class="fas fa-trash"></i>
                        </button>
                    </td>
                </tr>
                <?php endforeach; ?>
        ...
        <form method="POST" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="description" name="description" rows="1" 
                              class="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700">
                </div>
            </div>
...
<div id="editMailModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold">تعديل بريد صادر</h3>
        ...
        <form method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="edit_to_branch_id" class="block mb-2 font-medium">الجهة</label>
                    <select id="edit_to_branch_id" name="to_branch_id" required 
                            class="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700">
                        <option value="">اختر الفرع</option>
                        <?php foreach ($branches as $branch): ?>
                            <option value="<?= $branch['id'] ?>"><?= $branch['name'] ?></option>
                ...
                <div>
                    <label for="edit_sender_name" class="block mb-2 font-medium">مرسل من</label>
                ...
                <div>
                    <label for="edit_receiver_name" class="block mb-2 font-medium">مرسل الى</label>
...
function openEditModal(id, date, to_branch_id, code, description, sender_id, sender_name, receiver_id, receiver_name, handed_to) {
    document.getElementById('edit_id').value = id;
    document.getElementById('edit_date').value = date;
    document.getElementById('edit_to_branch_id').value = to_branch_id;
