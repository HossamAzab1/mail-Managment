...
// Prepare branch-wise data for charts - MODIFIED VERSION
...
$color_index = 0;
foreach ($branch_incoming_datasets as $branch => $monthly_data) {
    $branch_incoming_datasets[] = [
        'label' => $branch,
...
$color_index = 0;
foreach ($branch_outgoing_datasets as $branch => $monthly_data) {
    $branch_outgoing_datasets[] = [
        'label' => $branch,
        'data' => array_values($monthly_data), // Ensure sequential array
        'backgroundColor' => $color_palette[$color_index % count($color_palette)],
